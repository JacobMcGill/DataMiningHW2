...
```{r, include = FALSE}
engineered_reg = children ~ hotel+required_car_parking_spaces + 
                              total_of_special_requests + meal + poly(adults, 2)
                              + is_repeated_guest + summer
```

```{r, include = TRUE}
engineered_model = train(children ~ market_segment+ poly(adults,2) + customer_type                                               +is_repeated_guest + summer + lead_time 
                                   +previous_cancellations + previous_bookings_not_canceled 
                                   + previous_cancellations*previous_bookings_not_canceled 
                                  + stays_in_weekend_nights + meal + market_segment 
                                   ...
                                   + required_car_parking_spaces                                                                 +  poly(total_of_special_requests, 2), 
                  data=hotels_dev, 
                  method = "lm", 
                  trControl = train.control)
